...
func (m *postgresDBRepo) InsertRoomRestriction(r models.RoomRestriction) error {
	...
	_, err := m.DB.ExecContext(ctx, stmt, r.StartDate, r.EndDate, r.RoomID, r.RerservationID, time.Now(), time.Now(), r.RestrictionID)
...
// SearchAvailabilityByDatesByRoomID returns true if availability esists for roomID, and false if no availability
...
// UpdateUser updated a user in the database
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `udate user set 
		first_name = $1, 
		last_name = $2, 
		email = $3, 
		access_level = $4, 
